...
#log counts
LCountsMmus <- log10(nCountsMmus+1)
LCountsERCC <- log10(nCountsERCC+1)
 ...
 LmeansMmus <- rowMeans( LCountsMmus )
 LvarsMmus <- rowVars( LCountsMmus )
 Lcv2Mmus <- LvarsMmus / LmeansMmus^2
...
x11()
#pdf('./Mmus_mean_CV81blue.pdf',width=4.5,height=4.5)
...
sym_names_het=sym_names[which(padjA<0.1)] 
